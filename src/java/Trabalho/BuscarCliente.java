    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            /* TODO output your page here. You may use following sample code. */
             String filtro = request.getParameter("filtroCpf");

            ClienteDAO ClienteDAO = new ClienteDAO();
            List<Cliente>lstcliente = new ArrayList();

            try {
                lstcliente= ClienteDAO.getLista(filtro);
                
                request.setAttribute("dados", lstcliente);
                RequestDispatcher rd = getServletContext().getRequestDispatcher("/cliente.jsp");
                rd.forward(request, response);
                
          
            } catch (SQLException ex) {
                Logger.getLogger(ProcessaLogin.class.getName());
        }
        }
    }
